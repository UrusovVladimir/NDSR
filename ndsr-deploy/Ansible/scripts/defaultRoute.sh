...
if [[ "$1" == "host" ]]; then
  for models in $model; do
    ...
    if [[ $current ]]; then
      ...
      docker exec "$models" sudo ip r add default via 10.10.21.1 dev eth0
      echo "$models default route $current has been changed to via 10.10.21.1 dev eth0"
    fi
  done
else
  for models in $model; do
    ...
    if [[ $current ]]; then
      docker exec "$models" sudo ip r del default
      docker exec "$models" sudo ip r add default via 192.168.1.1 dev eth0
      echo "$models default route $current has been changed to via 192.168.1.1 dev eth0"
    fi
  done
fi
