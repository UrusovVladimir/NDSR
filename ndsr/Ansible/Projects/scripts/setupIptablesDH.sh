...
:
        ...
        sudo iptables -t nat -A PREROUTING -i internet -p tcp --dport ${WEB_PORTS[$MODEL]} -j DNAT --to-destination ${IPv4_ADDRESS[$MODEL]}:${WEB_PORTS[$MODEL]}
fi

if  sudo iptables -t nat -S -v | grep -E -q  "to-destination ${IPv4_ADDRESS[$MODEL]}:${SSH_PORTS[$MODEL]}"; then
:
else
         sudo iptables -t nat -A PREROUTING -i internet -p tcp --dport ${SSH_PORTS[$MODEL]} -j DNAT --to-destination ${IPv4_ADDRESS[$MODEL]}:${SSH_PORTS[$MODEL]}
fi

if  sudo iptables -t nat -S -v | grep -E -q  "to-destination ${IPv4_ADDRESS[$MODEL]}:${VNC_PORTS[$MODEL]}"; then
:
else
        sudo iptables -t nat -A PREROUTING -i internet -p tcp --dport ${VNC_PORTS[$MODEL]} -j DNAT --to-destination ${IPv4_ADDRESS[$MODEL]}:${VNC_PORTS[$MODEL]}
fi
